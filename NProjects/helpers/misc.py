...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("NProjects").info(f"Git Client Found")
    except GitCommandError:
        LOGGER("NProjects").info(f"Invalid Git Command")
        ...
        LOGGER("NProjects").info("Fetched Latest Updates")
    ...
    if is_heroku:
        if HEROKU_API_KEY and HEROKU_APP_NAME:
            try:
                ...
                LOGGER("NProjects").info(f"Heroku App Configured")
...
async def create_botlog(client):
    ...
    desc = "Group Log untuk NK-Userbot.\n\nHARAP JANGAN KELUAR DARI GROUP INI.\n\n✨ Powered By ~ @NProjectSTD ✨"
    try:
