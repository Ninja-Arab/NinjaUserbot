...
from NProjects.helpers.tools import get_arg, get_text, resize_media
from NProjects.utils.tools import add_text_img, bash
...
async def kang(client: Client, message: Message):
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            return
        media_ = await client.download_media(replied, file_name="NProjects/resources/")
    else:
...
async def tinying(client: Client, message: Message):
    ...
    im1 = Image.open("NProjects/resources/blank.png")
    if ik.endswith(".tgs"):
