...
from NProjects import TEMP_SETTINGS
from NProjects.helpers.SQL.devs import DEVS
from NProjects.helpers.basic import edit_or_reply
from NProjects.helpers.SQL.globals import addgvar, gvarstatus
...
async def incomingpm(client: Client, message: Message):
    try:
        ...
        from NProjects.helpers.SQL.pm_permit_sql import is_approved
...
async def auto_accept(client, message):
    try:
        from NProjects.helpers.SQL.pm_permit_sql import approve, is_approved
    except BaseException:
        pass
...
async def approvepm(client: Client, message: Message):
    try:
        from NProjects.helpers.SQL.pm_permit_sql import approve
    except BaseException:
...
async def disapprovepm(client: Client, message: Message):
    try:
        from NProjects.helpers.SQL.pm_permit_sql import dissprove
    except BaseException:
        await message.edit("Running on Non-SQL mode!")
        return
...
async def setpm_limit(client: Client, cust_msg: Message):
    ...
    try:
        from NProjects.helpers.SQL.globals import addgvar
